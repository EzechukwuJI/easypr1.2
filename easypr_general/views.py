...
def  createUserAccount(request):
    ...
    if  request.method            ==     "POST":
        ...
        if User.objects.filter(email = rp['email']).exists():
            msg  =   "Sorry,  %s  is already registered on this site." %(rp['email'])
            ...
            if user_account_form.is_valid:
                ...
                if request.POST.get('redirect_to'):
                    messages.info(request, "Your Account has been created succesfully. Complete your registration with confirmation link sent to your email.")
        ...
        if loginForm.is_valid():
            ...
            if auth_user is not None:
                ...
                if user.is_active:
                    ...
                    if request.POST.get('next') and not request.POST['next'] == "":
                    ...
                    if user.is_staff or user.is_superuser:
                        ...
                        if request.POST.get('redirect_to'):
                            ...
                            return redirect('/')
                            # return redirect(reverse('easypr_ng:user-dashboard', kwargs={'section_title':'newsroom'}))
                else:
