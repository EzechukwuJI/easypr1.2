...
from easypr_ng.models import PressMaterial, Package # EmailTemplateImages
from django.shortcuts import get_object_or_404
from easypr.settings import DEFAULT_FROM_EMAIL, BASE_DIR
...
def transaction_ref(transaction_type, model, stringlength):
    ...
    if transaction_type == "request":
        return trx_id if not model.objects.filter(ticket_number = trx_id).exists() else transaction_ref(transaction_type, model, stringlength)    
    return trx_id if not model.objects.filter(transaction_id = trx_id).exists() else transaction_ref(transaction_type, model, stringlength)	
...
#     print "recipients ", kwargs.get('useremail')
#     ctx        = {
#         'username': name,
...
#     if extra_ctx.has_key('files_to_attach'):
#         for file in extra_ctx['files_to_attach']:
#             msg.attach(file.image.name, file.image.read()) # attach uploaded file to mails
#     try:
#         print "Sending mail ..."
#         msg.send()
...
# email_images  =   EmailTemplateImages.objects.filter(active = True)
...
def easypr_send_mail(request, **kwargs):
    ...
    for f in ['easyprlogo.png', 'twitter.png', 'facebook.png', 'youtube.png','instagram.png','linkedin.png']:
        fp = open(os.path.join(BASE_DIR, 'static/easypr/images/email_img', f), 'rb')
